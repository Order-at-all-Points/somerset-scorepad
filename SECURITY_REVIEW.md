...
| 8 | Low | `people/$personId/linkedUids/$uid` write still isn't gated by proof of a valid, unexpired `linkCodes` entry — a device can join any group it can name, bypassing the intended one-time-code flow | **Fix drafted** — proof-of-code `.validate` in FIREBASE_SETUP.md §4, client already forward-compatible; **pending manual rules publish** |
| 9 | Low | `personOf/$uid` is world-readable to any signed-in client — a uid→personId oracle that undercuts #8's "personId is never exposed" defense | **Fix drafted** — `.read` tightened to own-uid in FIREBASE_SETUP.md §4; **pending manual rules publish** |
| 10 | Low | Link codes are multi-use and unrevocable for their whole 30-minute window; redemption never consumes them, and dead codes accumulate forever | **Partially drafted** — rules now permit any-auth delete (FIREBASE_SETUP.md §4, pending publish); client-side consume-on-redeem/owner cleanup still open |
| 11 | Low | Redeeming a maliciously supplied code silently and (in-app) irreversibly links your History to a stranger — no confirmation, no unlink flow | **Open** (2026-07-13 review) — unlink UI is the missing piece; rules already permit it |
| 12 | Info | No `.validate` on values under `users/$uid` or `people/$personId/linkedUids/$uid` — unbounded blobs, fanned out to every linked peer's open listener | **Fix drafted** — `linkedUids` value validated by #8's proof-of-code rule; history records require `id`+`date` (FIREBASE_SETUP.md §4, pending publish) |
...
### 9. Low — `personOf/$uid` is world-readable: a uid → personId oracle (Open)
...
**Fix:** tighten to `".read": "auth != null && auth.uid === $uid"` (or remove `.read` entirely — deny-by-default). Nothing in the app breaks; verified against every `personOf` reference in `index.html`.

### 10. Low — Link codes are multi-use, unrevocable, and immortal (Open)

`redeemLinkCode` never deletes the code it redeems, and the write rule (`!data.exists() || data.child('ownerUid').val() === auth.uid`) means the **redeemer couldn't delete it even best-effort** — only the owner can. `generateLinkCode` never cleans up its own codes either (each tap mints a new one), and RTDB has no TTL — the 30-minute window only gates *reads*, so expired codes pile up as permanent garbage rows.

**Failure scenario:** the intended one-device-shows-another UX treats the code as single-use, but anyone who sees it within 30 minutes — shoulder-surf, a photo of the screen, pasted into the wrong chat after the "tap to copy" — can redeem it too, silently joining the person-group with ongoing read access to every linked device's history (and nothing in the UI ever shows group membership, so the extra member is invisible).

**Fix:** the generator device should watch `people/<pid>/linkedUids` and delete its outstanding `linkCodes/<code>` on the first new join and again when the "show code" sheet closes (it needs that subscription anyway — see correctness finding C2 below). Optionally also let a redeemer consume the code at the rules level by allowing authenticated *deletes*: `".write": "auth != null && (!newData.exists() || !data.exists() || data.child('ownerUid').val() === auth.uid)"` — deleting requires already knowing the code, and knowing the code already grants a join, so this adds no new attack surface.

### 11. Low — Redeeming a hostile code silently links your History to a stranger, with no unlink flow (Open)

`redeemLinkCode` trusts whatever `personId` the code carries. There is no confirmation of *whose* group you are joining, no display of group membership anywhere in the UI afterward, and **no unlink flow at all** (the overnight brief asked whether that's a gap: for this reason, yes).

**Failure scenario:** an attacker generates a code on their own device and social-engineers the victim into typing it — "enter this code and our stats will merge for the tournament." The victim's entire game History becomes readable by the attacker from that moment on (silently, indefinitely), the attacker's records pollute the victim's Personal Stats, and the victim cannot undo it in-app — recovery means hand-editing Firebase data and localStorage.

**Fix (two independent pieces):** (a) an **unlink** control — the DB primitives already exist and the rules already permit them (delete own `people/<pid>/linkedUids/<uid>` entry; `personOf/$uid`'s `.validate` explicitly allows deletion via `!newData.exists()`), so this is UI work only: clear the two server entries plus local `personId`/`linkedUids`/`cloudSyncEnabled`, detach listeners. (b) A confirmation step on redeem that shows something identifying the group's owner (e.g. the generator's device name stored into the `linkCodes` record) before committing.

### 12. Info — No value validation under `users/$uid` or `linkedUids/$uid` (Open)

`users/$uid` has no `.validate` at all, and `people/$personId/linkedUids/$uid` accepts any value (`true` is convention, not rule). A client can store arbitrarily large blobs under its own uid's history or its own `linkedUids` entry. Same family as finding #5, with one new twist: linked peers hold **open `on("value")` listeners** on both paths (`subscribeLinkedHistories`), so one member's oversized write is repeatedly downloaded by every other member — a bandwidth/quota amplifier inside the trust group, not just abstract quota abuse.

**Fix:** `.validate` `newData.val() === true` on `linkedUids/$uid` (or the code-value check from #8's second opinion, which subsumes it); a shape check on history records (`newData.hasChildren(['id','date','teams','winner'])`) keeps garbage out of `users/$uid` even though RTDB can't express a real size cap.
...
| C4 | Low | Deleting a shared-match record doesn't stick: the linked device's copy resurrects it in merged Stats | Open (tombstones; would also close C1's offline-delete caveat) |
| C5 | Low | Casual record ids (`Date.now()`, no random part) are now cross-device dedup keys — same-millisecond archives on two linked devices silently drop a game | Open |
...
### C4. Low — Deletion of a shared match doesn't survive the merge

Deleting a shared-match record on device A removes it locally and from A's cloud path — but the linked device B archived its *own* copy of the same match (same `matchUid`, different id) under its own uid, which A's `mergedHistoryForStats` happily re-includes. Net effect: the user deletes a game, watches it disappear from the Log, and it stays in their Stats forever (and would reappear in any future merged-Log view). Only deleting on every linked device makes it stick.

**Fix:** propagate deletion by `matchUid` — e.g. keep the C1 tombstone list keyed by `matchUid` where present and have `mergedHistoryForStats` skip tombstoned matches; or accept and document "shared matches can only be fully removed from the device that owns each copy."

### C5. Low — `Date.now()`-only ids became cross-device dedup keys

`archiveCurrentGame` (`var id = Date.now()`) and `archiveManualWin` (`id: Date.now()`) have no random component — fine when ids only had to be unique within one device's array, but `mergedHistoryForStats` now uses `r.matchUid || r.id` as a **cross-device** dedup key. Two linked devices archiving casual (non-shared) games in the same millisecond collide, and one legitimate game silently vanishes from merged Stats. `buildHistoryRecordForMatch` already does `Date.now() + Math.floor(Math.random() * 1000)`; the two older archive paths should match it (and a `"m:" + matchUid` / `"i:" + id` key prefix in `mergedHistoryForStats` would make the two key namespaces collision-proof by construction rather than by the accident of matchUids containing `:`).
