...
- `statsProfiles/$profileId` — backs the opt-in "Stats Sharing" feature: a compact, self-published **aggregate digest** (W/L, moons, sets, streaks, recent form) that people you've played with can follow. Deliberately NOT a history mirror — a digest never contains individual games, scores, dates, or third-party names, so following someone reveals only their own career line. Access model:
