...
- `statsProfiles/$profileId` — backs the opt-in "Stats Sharing" feature: a compact, self-published **digest** (W/L, moons, sets, streaks, recent form, plus the owner's few **highlight games** — biggest win / nailbiter / longest / shortest — as full hand-by-hand records) that people you've played with can follow. Deliberately NOT a history mirror — only those select games leave the device, and **third-party names are stripped from them by default** (rosters blank, team labels neutralized; deals and scores still render): the people in someone's games never picked that person's followers, so naming them is a separate, explicit opt-in ("Include names in shared games"), meant for closed groups that all know each other. Access model:
