...
- `statsProfiles/$profileId` — backs the opt-in "Stats Sharing" feature: a compact, self-published **digest** (W/L, moons, sets, streaks, recent form, plus a capped recent-games feed of results/scores/dates) that people you've played with can follow. Deliberately NOT a history mirror — hand-by-hand logs never leave the device, and **third-party names are excluded from the recent-games feed by default**: the people in someone's games never picked that person's followers, so naming them is a separate, explicit opt-in ("Include names in shared games"), meant for closed groups that all know each other. Access model:
