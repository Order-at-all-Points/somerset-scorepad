...
- `tournaments/$code` — **the read and write windows are deliberately different lengths** (both anchored to `_createdAt`):
  ...
  - Known wart: joining a session that's past the write window succeeds and shows "Live", but any attempt to record a hand fails the lock transaction and surfaces the misleading "Someone else just started entering this hand." There's no read-only state in the client yet; rejoining to repair History is unaffected.
